...
describe('Turnstile gate policy (passesTurnstile)', () => {
  ...
  it('verifies native attestation when secret is set and attestation required, but bypasses when no secret', async () => {
    process.env.NATIVE_ATTESTATION_REQUIRED = '1';
    try {
      const native = fakeReq({ origin: 'capacitor://localhost' }, '203.0.113.55');
      // With no secret set (e.g. domestic/ESA edge protection or dev), requests pass through
      await expect(passesTurnstile(native, {}, '')).resolves.toBe(true);
      // With secret set, native arm enforces attestation proof
      await expect(passesTurnstile(native, {}, testSecret)).resolves.toBe(false);
